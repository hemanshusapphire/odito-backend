 ...
 * GEO Hub Routes
 *
 * Hub-scoped endpoints for Generative Engine Optimization.
...
import { getGEOHubData, getAISearchAuditIssuePages } from '../../services/hub/geoHubAggregation.js';
...
/**
 * GET /hub/geo/projects/:projectId
 * Hub-scoped dashboard metrics + issues for the GEO Hub.
 */
router.get('/projects/:projectId', validateProjectAccess(), async (req, res) => {
  ...
  try {
    const data = await getGEOHubData(projectId);
    ...
    return res.status(500).json({ success: false, message: 'Failed to get GEO Hub data', error: error.message });
  }
});

/**
 * GET /hub/geo/projects/:projectId/issues/:issueId/affected-pages
 */
