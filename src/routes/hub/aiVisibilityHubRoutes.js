/**
 * AI Visibility Hub Routes
 *
 * Additive hub-scoped endpoints that run parallel to the legacy
 * /ai-visibility/projects/:projectId/ai-search-audit routes.
 * Legacy routes remain live during the 90-day overlap window.
 *
 * Base: /hub/ai-visibility
 */
...
import { getAIVisibilityHubData, getAISearchAuditIssuePages } from '../../services/hub/aiVisibilityHubAggregation.js';
...
/**
 * GET /hub/ai-visibility/projects/:projectId
 * Hub-scoped dashboard metrics + issues for the AI Visibility Hub.
 */
router.get('/projects/:projectId', validateProjectAccess(), async (req, res) => {
  ...
  try {
    const data = await getAIVisibilityHubData(projectId);
    ...
    return res.status(500).json({ success: false, message: 'Failed to get AI Visibility Hub data', error: error.message });
  }
});

/**
 * GET /hub/ai-visibility/projects/:projectId/issues/:issueId/affected-pages
 * Paginated affected pages for a specific rule issue.
 */
